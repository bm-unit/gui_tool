...
from .window import TGDrivesControlWindow
...
def _process_entry_point(channel, iface_name):
    logger.info('TGDrives control process started with PID %r', os.getpid())
    ...
    def get_frame():
        ...
        if received:
            if obj == IPC_COMMAND_STOP:
                logger.info('TGDrives control process has received a stop request, goodbye')
                app.exit(0)
            else:
                return obj

    win = TGDrivesControlWindow(get_frame, iface_name)
    win.show()

    logger.info('TGDrives control process %r initialized successfully, now starting the event loop', os.getpid())
    sys.exit(app.exec_())


# TODO: Duplicates PlotterManager; refactor into an abstract process factory
class TGDrivesControlManager:
